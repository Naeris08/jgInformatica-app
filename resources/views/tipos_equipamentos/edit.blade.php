    ...
    <div class="py-12 bg-slate-900 min-h-screen">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-slate-800/40 border border-slate-700 p-8 rounded-3xl shadow-2xl backdrop-blur-sm">
                ...
                <form action="{{ route('tipos_equipamentos.update', $tipo_equipamento->id) }}" method="POST" class="space-y-6">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Nome da Categoria</label>
                        <input type="text" name="name" value="{{ old('name', $tipo_equipamento->name) }}" required class="w-full bg-slate-900/50 border border-slate-700 text-slate-100 rounded-2xl px-4 py-3 focus:ring-indigo-500 focus:border-indigo-500" />
                    </div>
